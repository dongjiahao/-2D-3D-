...
class MenuTableViewController: UITableViewController {
    ...
    let TAGA_CELL_LABEL = 1
    let dataArr = ["多边形底面相似柱状体", "圆形底面相似柱状体"]
    let viewArr = ["ViewController"]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "\(dataArr[(indexPath as NSIndexPath).row])",
                                                 for: indexPath)
        let label = cell.viewWithTag(TAGA_CELL_LABEL) as! UILabel
        label.text = dataArr[(indexPath as NSIndexPath).row]
        return cell
    }
