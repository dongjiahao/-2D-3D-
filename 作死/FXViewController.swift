...
class FXViewController: UIViewController {
    ...
    func CM() {
        ...
        cm.completionBlock = {
            ...
            //侧面
            var matrix: Matrix<Bool> = self.CreateNewMatrix(self.initMatrix(边缘检测后的侧面!))
            
            //高宽比 = 矩阵的行数比上矩阵的列数
            rateHW = CGFloat(matrix.rows) / CGFloat(matrix.columns)
            边缘检测后的侧面 = nil
            
            //取宽度
            //决定分多少层
            for i in stride(from: 2, to: Int(matrix.rows) - 1, by: 2) {
    ...
    override func viewDidLoad() {
        ...
        //二维矩阵储存：像素化的图片再经筛选后的点
        //底面
        var matrix: Matrix<Bool> = CreateNewMatrix(initMatrix(边缘检测后的底面!))
        ...
        func addPoint(_ p: CGPoint) {
            if !checkP(p) {
                myCGPointArray.append(MyCGPoint(x: p.x, y: p.y, e: false))
            }
        }
        //储存顶点
        ...
        //平移因子
        //底面x方向所占最大宽度
        var xMax = simplePoint[0].x
        //例如：-------
        //          -----
        //xMax = 10
        //底面y方向所占最大宽度
        var yMax = simplePoint[0].y
        var xMin = simplePoint[0].x
        var yMin = simplePoint[0].y
